...
def editing(browser): # редактируем и проверка редактирования
    ...
    if not value_ed in input_editing_text_1:
...
class TestMain:
    def test_init(self, browser):
        ...
        input_text_todo = browser.find_element(By.XPATH, "//section[@class='space-column']//span")  #делаем туду выполненым
        input_text_todo.click()

        todo_delete = browser.find_element(By.CSS_SELECTOR, '[data-testid = "delete"]').click()  # удалить пункт туду
        ...
        button_hide = browser.find_element(By.CSS_SELECTOR, '[data-testid="change_visible"]') # Проверка названия кнопки Hide
        str_hide_text = button_hide.text
        if not "Hide" in str_hide_text:
        ...
        radio_type_todo = browser.find_element(By.XPATH, '//div[@class="options"]//span')# кнопка задизейблена до ввода текста (выбран тип)
        radio_type_todo.click()
        button_add = browser.find_element(By.CSS_SELECTOR, '[data-testid="add_todo"]')
